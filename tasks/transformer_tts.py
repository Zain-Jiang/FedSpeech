...
class TransformerTtsTask(BaseTask):
    ...
    def build_dataloader(self, dataset, shuffle, max_tokens=None, max_sentences=None,
                         required_batch_size_multiple=-1, endless=False, spk_mask=None):
        ...
        indices = dataset.ordered_indices_forTransfer(spk_mask)
    ...
    def after_infer(self, predictions):
        ...
        for num_predictions, prediction in enumerate(tqdm(predictions)):
            ...
            wav_pred = self.vocoder.mel2wav(outputs)
            self.saving_results_futures.append(
                self.saving_result_pool.apply_async(self.save_result, args=[
                    wav_pred, outputs, f'P', utt_id, text, gen_dir, None, encdec_attn, str_phs]))

            if targets is not None and hparams['save_gt']:
                wav_gt = self.vocoder.mel2wav(targets)
                self.saving_results_futures.append(
                    self.saving_result_pool.apply_async(self.save_result, args=[
    ...
    def save_result(wav_out, mel, prefix, utt_id, text, gen_dir,
                    pitch=None, alignment=None, str_phs=None, mel2ph=None):
