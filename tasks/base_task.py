...
class BaseDataset(torch.utils.data.Dataset):
    ...
    def ordered_indices_forTransfer(self, spk_mask=None):
        """Return an ordered list of indices. Batches will be constructed based
        on this order.(specified for transfer learning experiment)"""
        assert spk_mask is not None, 'Speak_mask does not exist.'
        assert len(spk_mask) == len(self), 'Speak_mask\'s length does not match the dataset\'s length.'

        index_mask = np.where(spk_mask == 0)[0]
        
        if self.shuffle:
            indices = np.random.permutation(len(self))
            indices = np.delete(indices, [index for index, item in enumerate(indices) if item in index_mask])
            if self.sort_by_len:
                indices = indices[np.argsort(np.array(self._sizes)[indices], kind='mergesort')]
        else:
            indices = np.arange(len(self))
            indices = np.delete(indices, index_mask)
        return indices
