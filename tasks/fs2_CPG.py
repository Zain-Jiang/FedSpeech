...
class Piggymask_Apply(torch.autograd.Function):
    @staticmethod
    def forward(ctx, model, piggymask_flatten, threshold):
        index = 0
        for name, parameters in model.named_parameters():
            mask_thresholded = Binarizer.apply(
                piggymask_flatten[index:index + torch.flatten(parameters).size()[0]].view(parameters.size()),
                threshold)
...
class Fs2CPGTask(FastSpeech2Task):
    ...
    def get_pruning_mask(self, model):
        masks = {}
        for name, parameters in model.named_parameters():
            if 'spk_embed_proj' in name or "piggymask" in name:
                continue
    ...
    def set_piggymask(self, model):
        ...
        self.piggymask_flatten = Parameter(torch.Tensor(flatten_size).fill_(0.01), requires_grad=True)

    def apply_piggy_old(self):
    ...
    def build_model(self):
        ...
        # load the mask
        if os.path.exists('./checkpoints/fs2_CPG_pruning_expand/pruning_mask.pkl'):
            print("Find an existed pruning mask, loading.")
            previous_pruning_mask = pickle.load(open('./checkpoints/fs2_CPG_pruning_expand/pruning_mask.pkl', 'rb'))
        ...
        # set piggymask and make requires_grad false
        if hparams['mode'] == "finetune":
            self.set_piggymask(model)
            for name, parameters in model.named_parameters():
                if 'spk_embed_proj' in name or "norm" in name:
            ...
            if hparams.get('piggy', -1) == "new":
                print("save piggy_mask_new")
                pickle.dump(self.piggymask_flatten,
                            open('./checkpoints/fs2_CPG_pruning_expand/piggy_mask_new' + str(
                                hparams['transfer_at_spkid'][0]) + '.pkl',
                                 'wb'))
            else:
                print("save piggy_mask_old")
                pickle.dump(self.piggymask_flatten,
                            open('./checkpoints/fs2_CPG_pruning_expand/piggy_mask' + str(
                                hparams['transfer_at_spkid'][0]) + '.pkl',
                                 'wb'))
        ...
        # get piggy mask when infer
        if hparams.get('piggy', -1) == "old" and hparams['mode'] == "infer":
            print("load_piggy_old")
            self.piggymask_flatten = pickle.load(
                open(
                    './checkpoints/fs2_CPG_pruning_expand/piggy_mask' + str(hparams['transfer_at_spkid'][0]) + '.pkl',
                    'rb'))
    ...
    def build_optimizer(self, model):
        ...
        self.optimizer = optimizer = torch.optim.AdamW(
            ...
            weight_decay=4e-5)
        if hparams['mode'] == "finetune":
            self.optimizer.add_param_group({'params': self.piggymask_flatten})
    ...
    def on_save_checkpoint(self, checkpoint):
        if hparams['mode'] == "prune":
            ...
            pickle.dump(self.pruning_mask, open('./checkpoints/fs2_CPG_pruning_expand/pruning_mask.pkl', 'wb'))
    ...
    def on_sanity_check_start(self):
        ...
        if hparams['mode'] == "finetune" and (hparams.get('piggy', -1) == -1 or hparams.get('piggy', -1) == ""):
            self.pruner.apply_mask()

    def _training_step(self, sample, batch_idx, _):

        if hparams['mode'] == "finetune" and (hparams.get('piggy', -1) == -1 or hparams.get('piggy', -1) == ""):
            self.apply_piggy_old()
    ...
    def validation_end(self, outputs):
        ...
        print(f"\n==============\n "
              ...
              f"\n==============\n")
        with open("valid_loss_CPG_girl_100_"+str(hparams['record_name'])+".txt", 'a+') as file_object:
    ...
    def test_step(self, sample, batch_idx):
        """Performs evaluation."""
        if hparams['mode'] == "infer" and hparams.get('piggy', -1) == "old":
            self.apply_piggy_old()
