...
class BaseTrainer:
    ...
    def restore(self, checkpoint_path, on_gpu):
        ...
        for name, param in checkpoint['state_dict'].items():
            if "_copy" in name or "piggy" in name:
                continue

            if len(curr_model_state_dict[name].size()) == 1:
                if curr_model_state_dict[name].size(0) < param.size(0):
            ...
            elif len(curr_model_state_dict[name].size()) == 2:
                if curr_model_state_dict[name].size(0) < param.size(0) or curr_model_state_dict[name].size(1)<param.size(1):
            ...
            elif len(curr_model_state_dict[name].size()) == 3:
                if curr_model_state_dict[name].size(0) < param.size(0) or curr_model_state_dict[name].size(1)<param.size(1) or curr_model_state_dict[name].size(2)<param.size(2):
                    curr_model_state_dict[name].copy_(param[:curr_model_state_dict[name].size(0), :curr_model_state_dict[name].size(1), :curr_model_state_dict[name].size(2)])
                else:
                    curr_model_state_dict[name][:param.size(0), :param.size(1), :param.size(2)].copy_(param)
            else:
                try:
    ...
    def restore_model(self, checkpoint_path, on_gpu):
        checkpoint = torch.load(checkpoint_path, map_location='cpu')
