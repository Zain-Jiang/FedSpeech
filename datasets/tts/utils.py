...
from datasets.tts.parse_textgrid import remove_empty_lines, TextGrid
...
def get_mel2ph(tg_fn, ph, mel, hparams):
    ...
    split = np.zeros(len(ph_list) + 1, np.float)
    tg_idx = 0
    ph_idx = 0
    ...
    for x in tg_align:
        x['xmin'] = float(x['xmin'])
        x['xmax'] = float(x['xmax'])
    assert len([x for x in tg_align if x['text'] != '']) == len([x for x in ph_list if x != '|'])
    while tg_idx < len(tg_align):
        ph = ph_list[ph_idx]
        x = tg_align[tg_idx]
        if x['text'] == '' and ph != '|':
            if tg_idx + 1 < len(tg_align):
                tg_align[tg_idx + 1]['xmin'] = (x['xmin'] + x['xmax']) / 2
            tg_idx += 1
        elif x['text'] != '' and ph == '|':
            ph_idx += 1
        else:
            assert (x['text'] == '' and ph == '|') \
                   or x['text'].lower() == ph.lower() \
                   or x['text'].lower() == 'punc', (x['text'], ph)
            split[ph_idx] = x['xmin']
            if ph_idx > 0 and split[ph_idx - 1] == 0 and ph_list[ph_idx - 1] == '|':
                split[ph_idx - 1] = split[ph_idx]
            ph_idx += 1
            tg_idx += 1
    assert tg_idx == len(tg_align), (tg_idx, [x['text'] for x in tg_align])
    assert ph_idx >= len(ph_list) - 1, (ph_idx, ph_list, len(ph_list), [x['text'] for x in tg_align])
